...
class MitoPipelineDashboard:
    def __init__(self, root):
        self.root = root
        self.root.title("MARKLaunch - Pipeline Dashboard")
        ...
        style.configure('.', background=bg_color, foreground=text_color, font=("Helvetica Neue", 11))
        style.configure('TNotebook', background=bg_color, borderwidth=0)
        style.configure('TNotebook.Tab', font=("Helvetica Neue", 12, "bold"), padding=[20, 10], background="#e2e8f0")
        ...
        style.configure('TLabelframe.Label', background=card_bg, font=("Helvetica Neue", 13, "bold"), foreground=accent_color)
        style.configure('TLabel', background=card_bg, font=("Helvetica Neue", 11))
        style.configure('TCheckbutton', background=card_bg)
        style.configure('TEntry', padding=5, font=("Helvetica Neue", 11))
        style.configure('TButton', font=("Helvetica Neue", 11, "bold"), padding=8)
        style.configure('Action.TButton', font=("Helvetica Neue", 12, "bold"), padding=12, background="#22c55e", foreground="white")
        style.map('Action.TButton', background=[('active', '#16a34a')])
        style.configure('Stop.TButton', font=("Helvetica Neue", 12, "bold"), padding=12, background="#ef4444", foreground="white")
        ...
        self.tab_pipeline = ttk.Frame(self.notebook, style='White.TFrame')
        self.tab_post = ttk.Frame(self.notebook, style='White.TFrame')
        ...
        self.log_text = scrolledtext.ScrolledText(self.log_frame, height=12, state='disabled', font=("Consolas", 10), bg="#f8fafc", fg="#0f172a")
    ...
    def init_pipeline_tab(self):
        ...
        script_frame.pack(fill='x', padx=15, pady=(15, 10))
        script_frame.columnconfigure(1, weight=1)

        ttk.Label(script_frame, text="Pipeline Script (.sh):").grid(row=0, column=0, sticky='e', padx=10, pady=15)
        self.script_path = tk.StringVar(value=self.get_def_path("MARK.sh"))
        ttk.Entry(script_frame, textvariable=self.script_path).grid(row=0, column=1, sticky='ew', padx=10)
        btn_browse_script = ttk.Button(script_frame, text="Browse & Load", command=self.browse_script)
        btn_browse_script.grid(row=0, column=2, padx=10)
        ...
        btn_frame = ttk.Frame(self.config_frame, style='White.TFrame')
        btn_frame.pack(fill='x', padx=10, pady=10)
        ttk.Button(btn_frame, text="Reset to Script Defaults", command=self.set_all_script_defaults).pack(side='left', padx=5)

        self.grid_frame = ttk.Frame(self.config_frame, style='White.TFrame')
        ...
        ttk.Label(io_frame, text="Linear Ref (FASTA):").grid(row=0, column=0, sticky='e', padx=10, pady=5)
        self.ref_path = tk.StringVar(value=self.get_def_path("linearized_mtdna.fasta"))
        ttk.Entry(io_frame, textvariable=self.ref_path).grid(row=0, column=1, sticky='ew', padx=10)
        ttk.Button(io_frame, text="Browse", command=lambda: self.browse_file(self.ref_path, [("FASTA", "*.fasta *.fa")])).grid(row=0, column=2, padx=10)

        ttk.Label(io_frame, text="Regions BED:").grid(row=1, column=0, sticky='e', padx=10, pady=5)
        self.bed_path = tk.StringVar(value=self.get_def_path("linearized_regions.bed"))
        ttk.Entry(io_frame, textvariable=self.bed_path).grid(row=1, column=1, sticky='ew', padx=10)
        ttk.Button(io_frame, text="Browse", command=lambda: self.browse_file(self.bed_path, [("BED", "*.bed")])).grid(row=1, column=2, padx=10)

        ttk.Label(io_frame, text="Adapter File:").grid(row=2, column=0, sticky='e', padx=10, pady=5)
        self.adapter_path = tk.StringVar(value=self.get_def_path("Updated_Adapter_Primer_List_Cutadapt_cleaned.txt"))
        ttk.Entry(io_frame, textvariable=self.adapter_path).grid(row=2, column=1, sticky='ew', padx=10)
        ttk.Button(io_frame, text="Browse", command=lambda: self.browse_file(self.adapter_path, [("Text", "*.txt")])).grid(row=2, column=2, padx=10)

        ttk.Label(io_frame, text="Input FASTQ Folder:").grid(row=3, column=0, sticky='e', padx=10, pady=5)
        self.input_folder = tk.StringVar()
        ttk.Entry(io_frame, textvariable=self.input_folder).grid(row=3, column=1, sticky='ew', padx=10)
        ttk.Button(io_frame, text="Browse", command=lambda: self.browse_folder(self.input_folder)).grid(row=3, column=2, padx=10)

        ttk.Label(io_frame, text="Output Base Dir:").grid(row=4, column=0, sticky='e', padx=10, pady=(10, 0))
        self.output_folder = tk.StringVar()
        ttk.Entry(io_frame, textvariable=self.output_folder).grid(row=4, column=1, sticky='ew', padx=10, pady=(10, 0))
        ttk.Button(io_frame, text="Browse", command=lambda: self.browse_folder(self.output_folder)).grid(row=4, column=2, padx=10, pady=(10, 0))

        ttk.Label(io_frame, text="(Optional: If left empty, outputs will save next to the input folder)", font=("Helvetica Neue", 10, "italic"), foreground="#64748b").grid(row=5, column=1, sticky='w', padx=10, pady=(0, 10))

        ttk.Label(io_frame, text="Custom Run Name:").grid(row=6, column=0, sticky='e', padx=10, pady=(5, 0))
        self.custom_run_name = tk.StringVar()
        ttk.Entry(io_frame, textvariable=self.custom_run_name).grid(row=6, column=1, sticky='ew', padx=10, pady=(5, 0))
        ttk.Label(io_frame, text="(Optional: If left empty, the pipeline uses its default naming convention)", font=("Helvetica Neue", 10, "italic"), foreground="#64748b").grid(row=7, column=1, sticky='w', padx=10, pady=(0, 10))

        ttk.Button(io_frame, text="Clear Fields", command=self.clear_pipeline_fields).grid(row=8, column=1, pady=15, sticky='w', padx=10)

        run_btn_frame = ttk.Frame(self.tab_pipeline, style='White.TFrame')
        run_btn_frame.pack(pady=15)

        self.btn_run_only = ttk.Button(run_btn_frame, text="RUN PIPELINE ONLY", style="Action.TButton", command=lambda: self.start_thread(self.run_pipeline_wrapper))
        self.btn_run_only.pack(side='left', padx=10)

        self.btn_run_auto = ttk.Button(run_btn_frame, text="RUN PIPELINE & POST-PROCESSING (AUTO)", style="Action.TButton", command=lambda: self.start_thread(self.run_automated_cycle))
        self.btn_run_auto.pack(side='left', padx=10)

        self.btn_stop = ttk.Button(run_btn_frame, text="STOP RUN", style="Stop.TButton", command=self.stop_pipeline, state='disabled')
        ...
        self.progress_bar = ttk.Progressbar(self.progress_frame, orient='horizontal', mode='determinate', length=500)
        self.progress_bar.pack(fill='x', padx=20, pady=(20, 10))
        self.progress_var = tk.StringVar(value="Idle")
        ttk.Label(self.progress_frame, textvariable=self.progress_var, font=("Helvetica Neue", 12, "bold"), foreground="#0ea5e9").pack(pady=(0, 20))
    ...
    def refresh_config_ui(self):
        ...
        for col, text in enumerate(headers):
            ttk.Label(self.grid_frame, text=text, font=("Helvetica Neue", 11, "bold")).grid(row=0, column=col, padx=10, pady=5, sticky='w')
        ...
        for var, dash_def in self.dash_defaults.items():
            ...
            ttk.Label(self.grid_frame, text=f"{var}:", font=("Helvetica Neue", 11, "bold")).grid(row=row, column=0, sticky='e', padx=5)

            color = "#64748b" if is_hardcoded else ("#0ea5e9" if clean_script_val != "Not Found" else "#94a3b8")
            ttk.Label(self.grid_frame, text=script_val, foreground=color, font=("Helvetica Neue", 11, "bold")).grid(row=row, column=1, padx=5, sticky='w')

            entry = ttk.Entry(self.grid_frame, textvariable=self.var_states[var]['entry_var'], width=12)
            ...
            chk = ttk.Checkbutton(self.grid_frame, variable=self.var_states[var]['override_var'], command=lambda v=var: [self.toggle_entry_state(v), self.update_inline_summary()])
            chk.grid(row=row, column=3, padx=10, sticky='w')
            self.var_states[var]['chk_widget'] = chk

            final_lbl = ttk.Label(self.grid_frame, text="", font=("Consolas", 11, "bold"))
    ...
    def update_inline_summary(self):
        ...
        for var, state in self.var_states.items():
            ...
            if clean_s_val == "Not Found":
                state['final_label'].config(text="Disabled (Not in script)", foreground="#94a3b8")
            elif state['override_var'].get():
                state['final_label'].config(text=f"➜ {val} (Override)", foreground="#e11d48")
            else:
                state['final_label'].config(text=f"➜ {clean_s_val} (Default)", foreground="#0ea5e9")
    ...
    def init_postproc_tab(self):
        ...
        frame_raw.pack(fill='x', padx=15, pady=10)

        ttk.Label(frame_raw, text="Run Folder:").pack(side='left', padx=10, pady=15)
        self.raw_main_dir = tk.StringVar()
        ttk.Entry(frame_raw, textvariable=self.raw_main_dir).pack(side='left', fill='x', expand=True, padx=10)
        ttk.Button(frame_raw, text="Browse", command=lambda: self.browse_folder(self.raw_main_dir)).pack(side='left', padx=10)
        ttk.Button(frame_raw, text="Run Step 1", style="Action.TButton", command=lambda: self.start_thread(self.collect_raw_wrapper)).pack(side='left', padx=10)

        frame_vcf = ttk.LabelFrame(self.tab_post, text="2. Linear VCF Correction")
        frame_vcf.pack(fill='x', padx=15, pady=10)
        ...
        ttk.Label(frame_vcf, text="Linearized Ref:").grid(row=0, column=0, sticky='e', padx=10, pady=10)
        ttk.Entry(frame_vcf, textvariable=self.vcf_mod_ref).grid(row=0, column=1, sticky='ew', padx=10)
        ttk.Button(frame_vcf, text="Browse", command=lambda: self.browse_file(self.vcf_mod_ref, [("FASTA", "*.fasta *.fa")])).grid(row=0, column=2, padx=10)

        ttk.Label(frame_vcf, text="Original Ref:").grid(row=1, column=0, sticky='e', padx=10, pady=10)
        ttk.Entry(frame_vcf, textvariable=self.vcf_orig_ref).grid(row=1, column=1, sticky='ew', padx=10)
        ttk.Button(frame_vcf, text="Browse", command=lambda: self.browse_file(self.vcf_orig_ref, [("FASTA", "*.fasta *.fa")])).grid(row=1, column=2, padx=10)

        ttk.Label(frame_vcf, text="VCF Folder:").grid(row=2, column=0, sticky='e', padx=10, pady=10)
        ttk.Entry(frame_vcf, textvariable=self.vcf_target_dir).grid(row=2, column=1, sticky='ew', padx=10)
        ttk.Button(frame_vcf, text="Browse", command=lambda: self.browse_folder(self.vcf_target_dir)).grid(row=2, column=2, padx=10)

        ttk.Button(frame_vcf, text="Run Step 2", style="Action.TButton", command=lambda: self.start_thread(self.vcf_correct_wrapper)).grid(row=3, column=1, pady=15)

        frame_bam = ttk.LabelFrame(self.tab_post, text="3. BAM Header Cleaning")
        frame_bam.pack(fill='x', padx=15, pady=10)

        self.bam_target_dir = tk.StringVar()
        ttk.Label(frame_bam, text="BAM Folder:").pack(side='left', padx=10, pady=15)
        ttk.Entry(frame_bam, textvariable=self.bam_target_dir).pack(side='left', fill='x', expand=True, padx=10)
        ttk.Button(frame_bam, text="Browse", command=lambda: self.browse_folder(self.bam_target_dir)).pack(side='left', padx=10)
        ttk.Button(frame_bam, text="Run Step 3", style="Action.TButton", command=lambda: self.start_thread(self.bam_clean_wrapper)).pack(side='left', padx=10)

        frame_final = ttk.LabelFrame(self.tab_post, text="4. Final Collection")
        frame_final.pack(fill='x', padx=15, pady=10)

        self.final_main_dir = tk.StringVar()
        ttk.Label(frame_final, text="Output Parent Folder:").pack(side='left', padx=10, pady=15)
        ttk.Entry(frame_final, textvariable=self.final_main_dir).pack(side='left', fill='x', expand=True, padx=10)
        ttk.Button(frame_final, text="Browse", command=lambda: self.browse_folder(self.final_main_dir)).pack(side='left', padx=10)
        ttk.Button(frame_final, text="Run Step 4", style="Action.TButton", command=lambda: self.start_thread(self.collect_final_wrapper)).pack(side='left', padx=10)

        ttk.Button(self.tab_post, text="Clear All Fields", command=self.clear_postproc_fields).pack(pady=20)
    ...
    def run_pipeline_wrapper(self):
        ...
        for v, s in self.var_states.items():
            if s['override_var'].get() and script_defaults.get(v, "Not Found").replace(" (Hardcoded)", "") != "Not Found":
                overrides.append(v)
